...
def run_preflight(
    ...
    try:
        ...
        # 1) Page sizes & bleed – pass UI bleed_mm
        result.page_sizes = check_page_sizes(doc, cfg, target_bleed_mm=bleed_mm)

        # 2) Image DPI – all images returned
        all_images, total_count = check_image_dpi(doc, cfg)
        ...
        if safe_margin_mm > 0:
            result.safe_area = check_safe_area(doc, cfg, safe_margin_mm)
